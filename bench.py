...
def scan_jax(a, x):
    ...
    return h_last, y
impls.append(('Linear(JAX, Mine)', scan_jax))


@jax.pmap
def scan_gemma(a, x):
    y, h_last = scan.lru_linear_scan(x, a)
    return h_last, y
...
@jax.pmap
def scan_assoc(a, x):
    y, h_last = scan.lru_associative_scan(x, a)
    return h_last, y
impls.append(('Linear(Assoc, Gemma)', scan_assoc))
...
if __name__ == "__main__":
    ...
    a = jax.device_put(a, NamedSharding(mesh, P('batch',)))
    x = random.normal(
        rng + 1,
        (jax.device_count(), batch_size // jax.device_count(), length, channels),
    )
